...
local function addToggle(page, o)
	...
	local kbBtn
	if hasKb then
		kbBtn = addKeybindBtn(row, o.title, o.keybind, function() hit.Activated:Fire() end, UDim2.new(1,-132,.5,-9))
	end

	local status = new("TextLabel", {TextColor3=C.SUB, TextSize=12, Font=Enum.Font.GothamBold,
		BackgroundTransparency=1, Position=UDim2.new(1,-96-(hasKb and 40 or 0),0,0),
	...
	local track = new("Frame", {Size=UDim2.new(0,46,0,26),
		Position=UDim2.new(1,-56-(hasKb and 40 or 0),.5,0),
	...
	hit.Activated:Connect(function() set(not on) end)
...
local function addSlider(page, o)
	local row, hit = rowBase(page, 62)
	bubble(row, o.icon, o.color); title(row, o.title)

	local valLbl = new("TextLabel", {
		Text = tostring(o.default or o.min or 0),
		TextColor3 = C.TEXT,
		TextSize = 12,
		Font = Enum.Font.GothamBold,
	...
	local current = o.default or min
	...
	hit.InputBegan:Connect(function(inp)
		if inp.UserInputType == Enum.UserInputType.MouseButton1 or inp.UserInputType == Enum.UserInputType.Touch then
			...
			local trackSize = math.max(track.AbsoluteSize.X, 0.001)
			local percent = math.clamp((absX - trackAbs) / trackSize, 0, 1)
...
--[[ ========== CHARACTER SETUP ========== ]]
local humanoid, rootPart, normalSpeed = nil, nil, Config.Speed or 16
...
local function CreateAntiFall()
	if not rootPart or AntiFallPart then return end
	AntiFallPart = Instance.new("Part")
	AntiFallPart.Name = "AntiFall_Circle_800x800"
	AntiFallPart.Shape = Enum.PartType.Cylinder
	AntiFallPart.Size = Vector3.new(2, 800, 800)
	AntiFallPart.CFrame = CFrame.new(rootPart.Position.X, rootPart.Position.Y - 7, rootPart.Position.Z) * CFrame.Angles(0, 0, math.rad(90))
	AntiFallPart.Anchored = true
	AntiFallPart.CanCollide = true
	AntiFallPart.Material = Enum.Material.Glass
	AntiFallPart.Color = Color3.fromRGB(80, 210, 255)
	AntiFallPart.Transparency = 0.45
...
RunService.Heartbeat:Connect(function()
	if State.AntiFall and AntiFallPart and rootPart then
		AntiFallPart.CFrame = CFrame.new(rootPart.Position.X, rootPart.Position.Y - 7, rootPart.Position.Z) * CFrame.Angles(0, 0, math.rad(90))
	end
end)
...
-- Pet Filter inputs
local nameLbl = new("TextLabel", {Text="Name:", TextColor3=C.SUB, TextSize=11,
...
local mpsLbl = new("TextLabel", {Text="Min MPS:", TextColor3=C.SUB, TextSize=11,
...
-- MAIN PAGE
addToggle(mainPage, {icon="🏃", color=C.BLUE, title="AUTO RUN", keybind="R",
	callback=function(v)
		State.AutoRun = v
		notify(v and "Success" or "Info", "Auto Run", v and "Enabled" or "Disabled")
	end})

local espToggle = addToggle(mainPage, {icon="👁", color=C.ACC1, title="PLAYER ESP", keybind="T", default=State.ESP,
	callback=function(v)
...
addButton(mainPage, {icon="⚡", color=C.ACC1, title="BYPASS MENU", accent=true,
...
addSlider(combatPage, {icon="🚀", color=C.BLUE, title="SPEED", min=16, max=120, default=normalSpeed,
	callback=function(v)
		normalSpeed = v; Config.Speed = v; SaveConfig()
...
local ColorPresets = {
	{name="Purple", c=Color3.fromRGB(120,80,220)},
	{name="Blue",   c=Color3.fromRGB(64,118,255)},
	{name="Red",    c=Color3.fromRGB(220,80,80)},
...
for _, preset in ipairs(ColorPresets) do
	...
	sw.Activated:Connect(function()
		for _, c in ipairs(colorRow:GetChildren()) do
			if c:IsA("TextButton") then c:FindFirstChildOfClass("UIStroke").Thickness = 0 end
		end
		stroke.Thickness = 2
		C.ACC1 = preset.c
		Config.AccentColor = {math.floor(preset.c.R*255), math.floor(preset.c.G*255), math.floor(preset.c.B*255)}
		SaveConfig()
		grad(winStroke, C.ACC1, C.ACC2, 90)
		grad(logo, C.ACC1, C.ACC2, 45)
		grad(indicator, C.ACC1, C.ACC2, 35)
